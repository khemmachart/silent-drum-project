...
- (void) initNetworkCommunication {
	...
	CFStreamCreatePairWithSocketToHost(NULL, (CFStringRef)@"172.20.10.2", 80, &readStream, &writeStream);
...
- (void) playSound: (NSString *)message {
    ...
    if ([message rangeOfString:@": "].location != NSNotFound) {
        ...
        NSString *original  = [message componentsSeparatedByString:@": "][1];
        NSString *soundName = [original stringByReplacingOccurrencesOfString:@"\n" withString:@""];
        
        if ([command isEqualToString:@"playSound"]) {
...
//    if ([message rangeOfString:@"playSound: "].location != NSNotFound) {
//    }
}
